console.log("[ZFT] 👁️ v0.5.2 | Gaze of Two Minds feature automation loading");
...
Hooks.on(
  ...
  async (activity, usageConfig, dialogConfig, messageConfig) => {
    try {
      return await this.handlePreUseActivityDiagnostic(
        activity,
        usageConfig,
        dialogConfig,
        messageConfig
      ...
      ZFTFA.error(
        "❌ Gaze Origin | dnd5e.preUseActivity diagnostic failed",
        {
...
/**
 * Inspect the DnD5e activity-use context before usage is configured.
 *
 ...
 * @returns {Promise<boolean>}
 */
async handlePreUseActivityDiagnostic(
  activity,
  usageConfig,
  dialogConfig,
  messageConfig
  ...
  if (!origin.valid) {
    ZFTFA.warn(
      "⚠️ Gaze Origin | preUseActivity continued from the normal token because proxy resolution failed",
      {
        actor: actor.name,
        actorUuid: actor.uuid,
        reason: origin.reason
...
const usageWorkflowToken =
  usageWorkflow?.token?.document
  ?? usageWorkflow?.token
  ?? null;

ZFTFA.log(
  "🔎 Gaze Origin | preUseActivity workflow-token JSON",
  JSON.stringify(
    {
      ...
      workflowExists: Boolean(usageWorkflow),

      workflowKeys: Object.keys(
        usageWorkflow ?? {}
      ),

      workflowTokenName:
        usageWorkflowToken?.name
        ?? null,

      workflowTokenUuid:
        usageWorkflowToken?.uuid
        ?? null,

      workflowTokenId:
        usageWorkflow?.tokenId
        ?? null,

      workflowTokenUuidProperty:
  ...
  const flattenedUsageConfig = {
    keys: Object.keys(usageConfig ?? {}),

    token: usageConfig?.token?.name
      ?? usageConfig?.token?.document?.name
      ?? null,

    tokenUuid: usageConfig?.token?.uuid
      ?? usageConfig?.token?.document?.uuid
      ?? null,

    tokenId: usageConfig?.tokenId
      ?? null,

    tokenUuidProperty: usageConfig?.tokenUuid
      ?? null,

    actorUuid: usageConfig?.actorUuid
      ?? null,

    itemUuid: usageConfig?.itemUuid
      ?? null,

    activityUuid: usageConfig?.activityUuid
      ?? null,

    midiOptions: foundry.utils.deepClone(
      usageConfig?.midiOptions ?? null
    ),

    workflowOptions: foundry.utils.deepClone(
      usageConfig?.workflowOptions ?? null
    )
  };

ZFTFA.log(
  "🔎 Gaze Origin | dnd5e.preUseActivity flattened context JSON",
  JSON.stringify(
    {
      actor: actor.name,
      actorUuid: actor.uuid,
      ...
      usageConfig: flattenedUsageConfig
    },
    null,
    2
  )
  ...
  ZFTFA.log(
    "✅ Gaze Origin | dnd5e.preUseActivity diagnostic completed without modifying the activity"
  );
